...
private:
    struct BinaryNode {
        ...
        BinaryNode(const Comparable &theElement, Student<string> &theVal, BinaryNode *lt, BinaryNode *rt)
                : element{theElement}, student{theVal}, left{lt}, right{rt} {
            student.setFirstName(theVal.getFirstName());
            student.setLastName(theVal.getLastName());
            student.setDepartment(theVal.getDepartment());
            student.setGPA(theVal.getGPA());
        }
    };
